 ...
 * 
 * Modified by Benjamin Hugo: underlying storage can now be a min-heap or a max-heap by specifying option in constructor
 */
...
namespace PowerDataStructures
{
    ...
    public class PriorityQueue<TPriority, TValue> : ICollection<KeyValuePair<TPriority, TValue>>
    {
        ...
        /// </summary>
        public PriorityQueue(bool mustBeMaxHeap)
            : this(Comparer<TPriority>.Default)
        {
            _isMaxHeap = mustBeMaxHeap;
        }
        ...
        private int HeapifyFromEndToBeginning(int pos)
        {
            ...
            while (pos > 0)
            {
                ...
                if (_isMaxHeap ? _comparer.Compare(_baseHeap[parentPos].Key, _baseHeap[pos].Key) < 0 :
                    _comparer.Compare(_baseHeap[parentPos].Key, _baseHeap[pos].Key) > 0)
                {
        ...
        private void HeapifyFromBeginningToEnd(int pos)
        {
            ...
            while (true)
            {
                ...
                int left = 2 * pos + 1;
                int right = 2 * pos + 2;
                if (_isMaxHeap ? left < _baseHeap.Count && _comparer.Compare(_baseHeap[smallest].Key, _baseHeap[left].Key) < 0 :
                    left < _baseHeap.Count && _comparer.Compare(_baseHeap[smallest].Key, _baseHeap[left].Key) > 0)
                    smallest = left;
                if (_isMaxHeap ? right < _baseHeap.Count && _comparer.Compare(_baseHeap[smallest].Key, _baseHeap[right].Key) < 0 :
                    right < _baseHeap.Count && _comparer.Compare(_baseHeap[smallest].Key, _baseHeap[right].Key) > 0)
                    smallest = right;
